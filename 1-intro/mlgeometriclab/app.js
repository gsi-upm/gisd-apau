...
let seed=4,model='linreg',points=[],fitted=null,labelNoise=.0,showLabelErrors=true,displaySplit='all';
...
function draw(){ctx.clearRect(0,0,plot.width,plot.height);if(fitted&&model!=='kmeans')for(let py=0;py<plot.height;py+=8)for(let px=0;px<plot.width;px+=8){const x=px/plot.width*4.8-2.4,y=(plot.height-py)/plot.height*4.2-2.1;ctx.fillStyle=fitted.x(x,y)?'rgba(232,127,68,.075)':'rgba(34,91,154,.065)';ctx.fillRect(px,py,8,8)}ctx.strokeStyle='#dfe4df';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,mapY(0));ctx.lineTo(plot.width,mapY(0));ctx.moveTo(mapX(0),0);ctx.lineTo(mapX(0),plot.height);ctx.stroke();const palette=['#225b9a','#e87f44','#118b85','#8d5aa8','#b99a2e'];points.filter(p=>displaySplit==='all'||(displaySplit==='train'?p.train:!p.train)).forEach(p=>{const px=mapX(p.x),py=mapY(p.y),r=p.train?5:6;ctx.beginPath();ctx.arc(px,py,r,0,Math.PI*2);const colorIndex=model==='kmeans'&&fitted?fitted.assign(p.x,p.y):p.c;ctx.fillStyle=palette[colorIndex%palette.length];ctx.globalAlpha=p.train?1:.55;ctx.fill();ctx.globalAlpha=1;if(!p.train){ctx.strokeStyle='#657180';ctx.lineWidth=1;ctx.stroke()}if(showLabelErrors&&p.corrupted){ctx.beginPath();ctx.arc(px,py,r+3,0,Math.PI*2);ctx.strokeStyle='#111';ctx.lineWidth=2.2;ctx.stroke()}});if(model==='kmeans'&&fitted)fitted.centers.forEach((c,j)=>{ctx.strokeStyle=palette[j%palette.length];ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(mapX(c[0])-8,mapY(c[1])-8);ctx.lineTo(mapX(c[0])+8,mapY(c[1])+8);ctx.moveTo(mapX(c[0])+8,mapY(c[1])-8);ctx.lineTo(mapX(c[0])-8,mapY(c[1])+8);ctx.stroke()})}
...
function setLegend(){const legend=$('.legend');if(!legend)return;const en=window.LabLanguage?.value==='en';const n=en?'Class 1 (positive)':'Clase 1 (positiva)',z=en?'Class 0 (negative)':'Clase 0 (negativa)',tr=en?'Training data':'Datos de entrenamiento',te=en?'Test data':'Datos de prueba',mod=en?'Modified label':'Etiqueta modificada';legend.innerHTML=model==='kmeans'?(en?'Colors = k-Means clusters':'Colores = grupos de k-Means'):'<div class="legend-row"><b>'+tr+'</b><span><i class="dot b"></i>'+n+'</span><span><i class="dot a"></i>'+z+'</span><span><i class="noise-ring"></i>'+mod+'</span></div><div class="legend-row"><b>'+te+'</b><span><i class="dot b faded"></i>'+n+'</span><span><i class="dot a faded"></i>'+z+'</span></div>'}
...
function evaluate(){
  if(model==='kmeans'){const a=points.filter(p=>p.train),inertia=a.reduce((s,p)=>{const c=fitted.centers[fitted.assign(p.x,p.y)];return s+(p.x-c[0])**2+(p.y-c[1])**2},0),sil=silhouette();setMetricLabels('Inercia','Silhouette','');$('#metric1').textContent=inertia.toFixed(1);$('#metric2').textContent=sil.toFixed(2);$('#metric3').textContent='';const en=window.LabLanguage?.value==='en';$('#generalization').innerHTML=en?'<strong>Inertia:</strong> sum of squared distances to each centre; lower is better.<br><strong>Silhouette:</strong> compares closeness to its cluster with separation from others; values near 1 indicate compact, well-separated clusters.':'<strong>Inercia:</strong> suma de las distancias cuadráticas de cada punto a su centro; menor es mejor.<br><strong>Silhouette:</strong> compara la cercanía al propio grupo con la separación respecto a otros; valores cercanos a 1 indican grupos compactos y separados.';return}
  const tr=points.filter(p=>p.train&&p.trueC<2),te=points.filter(p=>!p.train&&p.trueC<2),M=classificationMetrics(te,'trueC'),MT=classificationMetrics(tr,'trueC');setMetricLabels('F1','Precisión','Recall (exhaustividad)');$('#metric1').textContent=M.f1.toFixed(2);$('#metric2').textContent=M.precision.toFixed(2);$('#metric3').textContent=M.recall.toFixed(2);const gap=MT.f1-M.f1;const comparison='<strong>F1 entrenamiento (clase real):</strong> '+MT.f1.toFixed(2)+' · <strong>F1 prueba:</strong> '+M.f1.toFixed(2)+(gap>.1?' → posible sobreajuste.':' → generalización razonable.')+(M.precision===1?' <strong>Precisión 1,00:</strong> no hay falsos positivos en prueba; aún puede haber falsos negativos.':'')+(M.recall===1?' <strong>Exhaustividad 1,00:</strong> se han detectado todos los ejemplos reales de la clase 1; también puede haber clase 1 en ambas zonas.':'');const noiseNote=labelNoise>0?' '+Math.round(labelNoise*100)+'% de las etiquetas de entrenamiento están modificadas: el modelo se entrena con ellas, pero estas métricas se calculan respecto a la clase real.':'';$('#generalization').innerHTML=comparison+noiseNote;
}
function resetMetrics(){if(model==='kmeans')setMetricLabels('Inercia','Silhouette','');else setMetricLabels('F1','Precisión','Recall (exhaustividad)');$('#metric1').textContent=$('#metric2').textContent=$('#metric3').textContent='—';if(model==='kmeans')$('#metric3').textContent='';const en=window.LabLanguage?.value==='en';$('#generalization').innerHTML=model==='kmeans'?(en?'<strong>Inertia:</strong> measures compactness.<br><strong>Silhouette:</strong> measures compactness and separation.':'<strong>Inercia:</strong> mide la compactación.<br><strong>Silhouette:</strong> mide la compactación y la separación.'):'Ejecuta el modelo para calcular F1, precisión y recall sobre el conjunto de prueba.'}
