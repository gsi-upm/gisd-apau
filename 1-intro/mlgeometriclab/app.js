...
function setLegend(){const legend=$('.legend');if(!legend)return;const en=window.LabLanguage?.value==='en',noiseKey=en?' <span class="noise-ring"></span> Black border: modified label':' <span class="noise-ring"></span> Borde negro: etiqueta modificada',splitKey=en?' <span class="split-solid"></span> solid: training <span class="split-faded"></span> translucent: test':' <span class="split-solid"></span> opaco: entrenamiento <span class="split-faded"></span> translúcido: prueba';legend.innerHTML=model==='kmeans'?(en?'Colors = k-Means clusters':'Colores = grupos de k-Means')+noiseKey+splitKey:'<span class="dot a"></span> '+(en?'class 0 (negative)':'clase 0 (negativa)')+' <span class="dot b"></span> '+(en?'class 1 (positive)':'clase 1 (positiva)')+noiseKey+splitKey}
